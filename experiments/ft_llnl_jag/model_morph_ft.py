...
def _override_with_absolute_positional_encoding(model, max_ar, max_patches, dim, emb_dropout, device):
    target = model.module if isinstance(model, nn.DataParallel) else model
    target.pos_encoding = AbsolutePositionalEncoding(
        max_ar=max_ar,
...
def morph_ft(model_variant='S', device='cpu',
    ...
    filters, dim, heads, depth, mlp_dim = model_size
    heads_xa = 32
    dropout = 0.1
    emb_dropout = 0.1
    ...
    if standalone == False:
        ...
        if use_absolute_positional_encoding:
            _override_with_absolute_positional_encoding(
                morph,
                max_ar=max_ar_order,
                max_patches=max_patches,
                dim=dim,
