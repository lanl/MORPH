...
from src.utils.data_preparation_fast_poseidon import FastARDataPreparer
from config.data_config import DataConfig
from src.utils.dataloaders.dataloaderchaos_poseidon import DataloaderChaos
from src.utils.normalization import RevIN
from src.utils.lr_schedulars import LRSched
from src.utils.trainers_poseidon import Trainer
from sota.poseidon.transform_to_from_poseidon import to_poseidon_input, undo_poseidon_output    
from src.utils.simple_plotting import plot_samples

# === new classes for poseidon ===
from sota.poseidon.scOT.model import ScOT
# ===============================
...
parser.add_argument('--mode', type=str, choices=['ft', 'tfs', 'infer'], default='ft',
...
parser.add_argument('--model_size', type=str, choices=['T', 'B', 'L'], 
                    default='T', help="Model size to use for finetuning")
parser.add_argument('--ft_ckpt', type=str, default=None,
                    help='Checkpoint filename inside models/poseidon_ft/<ft_dataset>/')
...
parser.add_argument('--inp_res', type=int, default=128, 
                    help="Input resolution for HEAT2D finetuning")
...
parser.add_argument('--ar_order', type=int, default=1, help="Auto-regressive order for data preparation")
...
# savepaths
savepath_results = os.path.join(project_root, "experiments", "results", "test", "poseidon_ft")
os.makedirs(savepath_results, exist_ok=True)

# savepaths
savepath_model = os.path.join(project_root, "models", "poseidon_ft")
...
model_name = f'Poseidon__{run_tag}'
ckpt_name = os.path.join(savepath_model, f'Poseidon-{args.model_size}')

if args.mode == "tfs":
    config = AutoConfig.from_pretrained(ckpt_name, local_files_only=True)
    ft_model = ScOT(config).to(device)          # random init (no pretrained weights loaded)
    print("→ Model initialized from config (true scratch)")

elif args.mode == "ft":
    ft_model = ScOT.from_pretrained(ckpt_name, local_files_only=True).to(device)
    print("→ Model initialized with pretrained weights (finetuning mode)")

elif args.mode == "infer":
    if args.ft_ckpt is None:
        raise ValueError("--ft_ckpt is required when --mode infer")

    config = AutoConfig.from_pretrained(ckpt_name, local_files_only=True)
    ft_model = ScOT(config).to(device)

    checkpoint_path = os.path.join(savepath_model, args.ft_dataset, args.ft_ckpt)
    ...
    ft_model.eval()
    print("→ Model loaded from finetuned checkpoint (inference mode)")
...
#######################################
if args.mode != "infer":
    # --- Fine-tuning setup (optimizers, loss, schedulars) ---
    # loss function
    criterion = nn.MSELoss()

    # optimizer and finetuning parameters
    optimizer = torch.optim.AdamW(ft_model.parameters(), lr=args.lr, weight_decay=args.wd)
...
with torch.no_grad():  
    for s in range(0, X_te.shape[0], batch_size):  
        ...
        # transform into poseidon input format
        inputs_, meta_in = to_poseidon_input(inputs, out_size=args.inp_res, pad_mode="replicate")
        t = torch.zeros((inputs_.shape[0], 1), device=inputs_.device)

        # pass through the model
        prediction = ft_model(pixel_values=inputs_, time=t, return_dict=False)[0]

        # transform the output back to original shape
        prediction_back = undo_poseidon_output(prediction, meta_in, take_channel=0)

        # squeeze dims
        prediction_sq = prediction_back[:, 0, :, :]  # (B,4,H,W)->(B,H,W)

        # store
        predictions_all.append(prediction_sq.detach().cpu())
predictions_all = torch.cat(predictions_all, dim = 0) #(N,H,W)
print(f'→ Overall Predictions: {predictions_all.shape}')

# get the original inputs and targets
inputs_all = X_te[:,0,:,:] # (N,H,W)
targets_all = y_te[:,0,:,:] # (N,H,W)
...
if args.viz_on:
    plot_samples(inputs_all, targets_all, predictions_all, savepath_results_, args, n=10,
                 filename = f'poseidon-{args.model_size}-{args.ft_dataset}')
