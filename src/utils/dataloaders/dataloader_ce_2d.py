...
def split_and_save_h5(raw_h5_loadpath, savepath, dataset_name='CE', 
    ...
    for fname in raw_files:
        ...
        with h5py.File(raw_path, 'r') as f_in:
            data = f_in['data'][:]  # shape: (N, 21, 2, 128, 128)
            file_attrs = dict(f_in.attrs)
            ds_attrs   = dict(f_in['data'].attrs)
        ...
        for split, idxs in splits.items():
            ...
            with h5py.File(out_path, 'w') as f_out:
                dset = f_out.create_dataset('data', data=data[idxs])
...
class CE2dDataLoader:
    def __init__(self, data_path, dataset_name='CE'):
    ...
    def load_split(self, split):
        ...
        for fname in files:
            ...
            with h5py.File(path, 'r') as f5:
                # read data
                data = f5['data'][:]   # (20000,21,5,128,128)
                ...
                print(f'Shape of concat data: {data.shape}')
